"""Copper Close-to-Close Direction Model (main).

THE question: will COMEX copper close higher TOMORROW than it closed TODAY?
A pure up/down call, every day — we do not care by how much.

    target_up = 1 if close_{t+1} > close_t else 0

This is the consolidated, self-contained successor to the earlier prototype chain.
What it keeps (the pieces that actually produced a statistically real edge):
  * Features  : copper price features + EXPANDED cross-asset DIVERGENCE
                (copper-miner / China / base-metal returns minus copper's own move).
  ...
  * Model     : Logistic Regression with isotonic probability calibration
                (it beat RF / GB / HistGB — the usable signal is linear).
  * Validation: expanding-window walk-forward, ~1,800 out-of-sample days.
...
Run:  python copper_close_direction_model.py
Outputs -> outputs/copper_close_direction/
"""
...
# Cross-asset tickers (yfinance). Divergence = asset return minus copper's own return.
# NB: producer-FX (CLP/AUD/PEN) and relative-value (SPY/GLD/USO) were tested and REVERTED
# (2026-06-26): they were redundant with VIX/DXY/oil/CNY and worsened signal-to-noise
# (acc 53.9%->53.0%, permutation p 0.000->0.05). See memory copper-v3-macro-signal.
DIVERGENCE_TICKERS = ["FCX", "SCCO", "COPX", "BHP", "RIO", "VALE",
                      "FXI", "MCHI", "ASHR", "KWEB",
                      "DBB", "PICK", "XME", "SLX", "CPER"]
...
CLOSE_CANDIDATES = ["close", "comex_close", "hg_close", "copper_close",
                    "copper_futures_close", "settlement", "settle", "settlement_price", "px_last"]
DATASET_KEYWORDS = ["copper", "comex", "futures", "hg", "price", "market", "merged", "features"]


def log(msg: str) -> None:
    print(f"[copper_close] {msg}", flush=True)
...
def find_dataset(root: Path):
    ...
    for path in sorted(root.rglob("*")):
        ...
        name_cu = any(k in path.stem.lower() for k in ("copper", "comex", "hg"))
        ...
        if score > best_score:
            ...
            reason = (f"Chosen `{path.name}` (score={score}): date + close ('{close}'), "
                      f"OHLC={ohlc}, copper-in-name={name_cu}.")
    if best is None:
