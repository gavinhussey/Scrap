"""Overfitting / forward-test diagnostics for the pruned L1 aluminum model.

Identical protocol to the copper diagnostics. We make method choices (L1, drop-list,
C-grid) while watching the walk-forward, so accuracy could be partly fit to that window.
Three checks:
...
Reuses the frozen pruned pipeline from aluminum_close_direction_model.py.
...
import aluminum_close_direction_model as cc

BASE = Path(__file__).resolve().parent
OUT = BASE / "outputs" / "aluminum_close_direction"
...
def build():
    ...
    Xc = cc.metal_features(df, meta["merged_external_cols"]).reset_index(drop=True)
