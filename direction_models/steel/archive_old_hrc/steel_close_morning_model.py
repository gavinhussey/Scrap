...
OUT_DIR = BASE_DIR / "outputs" / "steel_close_morning"
OVERNIGHT_CACHE = BASE_DIR / "data" / "external" / "macro_cache" / "overnight_prices.csv"

# Asian + European steelmakers whose home session closes BEFORE the ~1pm ET HRC settle,
# so their move is known at steel-prediction time (leak-free via shift(-1)).
#   BSL.AX  BlueScope Steel (Sydney, ~closes 2am ET)
#   5401.T  Nippon Steel (Tokyo)          600019.SS  Baoshan Iron & Steel (Shanghai)
#   0347.HK Angang Steel (Hong Kong)      TKA.DE     Thyssenkrupp (Frankfurt, ~11:30am ET)
#   SSAB-B.ST SSAB (Stockholm, ~11:30am ET)
OVERNIGHT_TICKERS = ["BSL.AX", "5401.T", "600019.SS", "0347.HK",
                     "TKA.DE", "SSAB-B.ST"]


def log(m): print(f"[steel_morning] {m}", flush=True)
...
def overnight_features(prices, close, dates) -> pd.DataFrame:
    ...
    if ov_shifted:
        ...
        cols["steelmakers_overnight_mean"] = agg.to_numpy()
        cols["steelmakers_overnight_div"] = (agg - copper_ret1).to_numpy()
    if ov_raw:
        ...
        cols["steelmakers_5d_mom"]      = roll5.shift(-1).to_numpy()
        cols["steelmakers_20d_mom"]     = roll20.shift(-1).to_numpy()
        cols["steelmakers_vol5"]        = vol5.shift(-1).to_numpy()
        cols["steelmakers_vol20"]       = vol20.shift(-1).to_numpy()
        cols["steelmakers_trend_5_20"]  = base._safe_div(roll5, roll20).shift(-1).subtract(1).to_numpy()
...
LME_CSV = BASE_DIR / "data" / "external" / "lme_steel.csv"  # absent: no LME steel feed
...
# ----------------------------------------------------------------------------
# China/Asia ferrous complex — steel's analog to copper's LME anchor.
# Populated by bloombergSteel.py. Each CSV optional; absent files are skipped, so the
# model runs today on free data and gains these once Bloomberg has been pulled.
# ----------------------------------------------------------------------------
CHINA_SERIES = [
    (BASE_DIR / "data" / "external" / "shfe_rebar.csv",       "shfe_rb_close",  "shfe_rebar"),
    (BASE_DIR / "data" / "external" / "shfe_hrc.csv",         "shfe_hrc_close", "shfe_hrc"),
    (BASE_DIR / "data" / "external" / "dce_iron_ore.csv",     "dce_io_close",   "dce_iron_ore"),
    (BASE_DIR / "data" / "external" / "dce_coking_coal.csv",  "dce_cc_close",   "dce_coking_coal"),
    (BASE_DIR / "data" / "external" / "sgx_iron_ore.csv",     "sgx_io_close",   "sgx_iron_ore"),
]


def china_features(df: pd.DataFrame) -> pd.DataFrame:
    """Overnight China/Asia ferrous complex (SHFE rebar/HRC, DCE iron ore/coking coal,
    SGX iron ore) — the closest steel has to copper's LME lead.

    Timing: SHFE/DCE day sessions close ~3 PM Beijing (~1-2 AM CT) and SGX iron ore settles
    in Singapore — all BEFORE the ~1 PM ET CME HRC settle. So row t's t+1 value is known at
    prediction time, and shift(-1) is leak-free (identical logic to lme_features).

    Per available series:
      <name>_overnight_ret  : day-(t+1) return, known by the morning of t+1
      <name>_overnight_z20  : that return in 20-day sigma units (regime context)
    Plus china_steel_composite: equal-weight mean of all available overnight returns.

    Each CSV optional; absent files skipped. Run bloombergSteel.py to populate them.
    """
    cols = {}
    overnight_rets = []
    for csv_path, price_col, name in CHINA_SERIES:
        if not csv_path.exists():
            continue
        s = _align_series(df, csv_path, price_col)
        ret = s.pct_change(1)
        vol20 = ret.rolling(20).std()
        cols[f"{name}_overnight_ret"] = ret.shift(-1).to_numpy()
        cols[f"{name}_overnight_z20"] = base._safe_div(ret.shift(-1), vol20).to_numpy()
        overnight_rets.append(ret.shift(-1))
        log(f"China: {name} {s.notna().sum()} rows")
    if len(overnight_rets) > 1:
        cols["china_steel_composite"] = pd.concat(overnight_rets, axis=1).mean(axis=1).to_numpy()
    if not cols:
        return pd.DataFrame(index=range(len(df)))
    log(f"China-complex features: {len(cols)} total")
    return pd.DataFrame(cols, index=range(len(df))).replace([np.inf, -np.inf], np.nan)
...
def build_panel():
    ...
    Xch = china_features(df).reset_index(drop=True)
    n_lme = Xl.shape[1]
    n_gap = Xg.shape[1]
    n_china = Xch.shape[1]
    X = pd.concat([Xc, Xd, Xo, Xl, Xg, Xch], axis=1)
    y = df["target_up"].to_numpy(int)
    return df, X, y, meta, reason, Xo.shape[1], n_lme, n_gap, n_china
...
def write_summary(ctx, path):
    ...
    md = f"""# Steel (CME HRC) Close Direction Model — MORNING-OF variant
...
Validated: permutation null (real {me['accuracy']:.3f} vs null ~0.50); a no-shift control
(same-day steelmaker return) scores {ctx.get('ctrl_acc', 'see overnight_shift_validation.csv'):.4f}
vs real {me['accuracy']:.4f}, isolating any gain to the t+1 session shift.
...
- {len(ctx['selected_features'])} features selected (L1, per-fold C={ctx['chosen_C']}) of
  {ctx['candidates']} candidates; HRC + cross-asset divergence + overnight steelmakers
  + HRC overnight (Globex) open gap{' + China complex (SHFE/DCE/SGX)' if ctx.get('n_china', 0) > 0 else ''}.
...
def main() -> int:
    ...
    df, X, y, meta, reason, n_over, n_lme, n_gap, n_china = build_panel()
    log(reason)
    log(f"Rows={len(y)} features={X.shape[1]} "
        f"(overnight={n_over} lme={n_lme} gap={n_gap} china={n_china}) up_rate={y.mean():.4f}")
    ...
    # No-shift control: validates that the edge comes from tomorrow's overnight steelmaker
    # session, not same-day. The China complex stays shifted (it is a genuine pre-settle
    # lead, like LME in copper); only the equity-overnight block is de-shifted here.
    log("--- No-shift control (same-day steelmaker return; isolates the overnight shift) ---")
    overnight_prices = fetch_overnight()
    Xo_ctrl = overnight_features_no_shift(
        overnight_prices, df["close"], df["date"]
    ).reset_index(drop=True)
    Xc_ctrl = base.steel_features(df, meta["merged_external_cols"]).reset_index(drop=True)
    ...
    Xch_ctrl = china_features(df).reset_index(drop=True)
    X_ctrl = pd.concat([Xc_ctrl, Xd_ctrl, Xo_ctrl, Xl_ctrl, Xg_ctrl, Xch_ctrl], axis=1)
