"""Steel Close-to-Close Direction Model (main).

THE question: will CME US HRC steel close higher TOMORROW than it closed TODAY?
...
metal-agnostic; only the target series and the cross-asset "cousins" change.
What it keeps (the pieces that produced a statistically real edge on copper):
  * Features  : HRC price features + EXPANDED cross-asset DIVERGENCE
                (steelmaker / iron-ore / base-metal returns minus HRC's own move).
                The divergence block is what lifted AUC off the 0.50 floor.
  ...
  * Dead-band : a DEPLOYABLE confidence gate — trust the call only when the model
                is sure (|p-0.5| large). Volatility/move-size gates did NOT deploy.

HONEST CAVEAT: copper's morning model hit ~85% because LME and COMEX are the SAME
arbitraged commodity 6 hours apart. Steel has no such anchor — US HRC is regionally
priced, tariff-walled, and cash-settles to a weekly CRU index. Expect the copper
NIGHT-BEFORE regime (~52-56%), plus a few points from the overnight steel complex.
...
Run:  python steel_close_direction_model.py
Outputs -> outputs/steel_close_direction/
"""
...
OUT_DIR = BASE_DIR / "outputs" / "steel_close_direction"
...
# Cross-asset tickers (yfinance). Divergence = asset return minus HRC's own return.
# Steel's "cousins" — US/intl steelmakers, iron-ore miners + iron-ore futures (the key
# input cost), and base-metal/mining sector ETFs. Thin/illiquid tickers are dropped at
# runtime (divergence_features skips any series with <100 non-null obs), so an over-
# inclusive list is safe — let L1 prune. US Steel (X) excluded: delisted 2025 (Nippon).
#   NUE/STLD/CLF/RS/CMC/WOR  US steelmakers     PKX/TX/SID/GGB  intl steelmakers
#   VALE/RIO/BHP/FMG.AX      iron-ore miners    TIO=F           SGX 62% Fe iron-ore future
#   MT  ArcelorMittal (global)                  SLX/XME/PICK/DBB  steel & metals ETFs
DIVERGENCE_TICKERS = ["NUE", "STLD", "CLF", "RS", "CMC", "WOR", "MT",
                      "PKX", "TX", "SID", "GGB",
                      "VALE", "RIO", "BHP", "FMG.AX", "TIO=F",
                      "SLX", "XME", "PICK", "DBB"]
...
CLOSE_CANDIDATES = ["close", "hrc_close", "steel_close",
                    "steel_futures_close", "settlement", "settle", "settlement_price", "px_last"]
DATASET_KEYWORDS = ["steel", "hrc", "futures", "price", "market", "merged", "features"]


def log(msg: str) -> None:
    print(f"[steel_close] {msg}", flush=True)
...
def find_dataset(root: Path):
    ...
    skip_dirs = {"outputs", "macro_cache", "__pycache__", ".git", ".venv"}
    ...
    for path in sorted(root.rglob("*")):
        ...
        name_cu = any(k in path.stem.lower() for k in ("steel", "hrc"))
        score = 3 + 2 + ohlc + sum(any(kw in c for c in cols) for kw in DATASET_KEYWORDS) + 2 * name_cu
        if score > best_score:
            best, best_score = path, score
            reason = (f"Chosen `{path.name}` (score={score}): date + close ('{close}'), "
                      f"OHLC={ohlc}, steel-in-name={name_cu}.")
    if best is None:
...
# ----------------------------------------------------------------------------
# Features: steel (HRC) price block
# ----------------------------------------------------------------------------
def steel_features(df: pd.DataFrame, external_cols: list[str]) -> pd.DataFrame:
...
def write_summary(ctx, path):
    ...
    md = f"""# Copper Close-to-Close Direction Model
...
def main() -> int:
    ...
    df = create_target(df)
    Xc = steel_features(df, meta["merged_external_cols"]).reset_index(drop=True)
    macro = fetch_macro()
